...
export default function Navbar({ programs = [] }) {
    ...
    const handleClickOutside = (event) => {
      ...
      if (!isMobileMenuOpen) {
        if (
          ...
          !ourProgramsDropdownRef.current.contains(event.target)
        ) {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 pt-4 left-0 right-0 z-50 bg-white h-16 md:h-20 transition-shadow duration-300 ${
