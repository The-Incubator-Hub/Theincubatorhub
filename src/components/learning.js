...
const SearchBar = ({ placeholder = "Search resources..." }) => (
  <div className="mx-auto mt-8 max-w-md w-full px-4">
    <div className="relative w-full">
      ...
      <input
        ...
        className="w-full rounded-lg border border-gray-300 bg-white py-3 pl-12 pr-4 text-sm sm:text-base shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 hover:border-gray-400 transition-all duration-200"
      />
    </div>
  </div>
);
...
const ResourceCard = ({ resource, index }) => {
  ...
  const styling = resource.highlighted && resource.styling ? resource.styling : defaultStyling;
  ...
  return (
    ...
    >
      <div className="flex-shrink-0">
        <div className={`inline-flex items-center justify-center p-2 sm:p-3 rounded-lg ${styling.iconBg} mb-2 mx-auto`}>
          {resource.icon}
        </div>
        <h3 className="font-bold text-gray-900 mb-1 text-xs sm:text-sm" style={{ lineHeight: '1.2' }}>
          {resource.title}
        </h3>
      </div>
...
export default function LearningResources({
  ...
  // Use props resources if provided, otherwise use defaults
    ...
    ? propsResources.map((resource) => {
        ...
        return {
          ...
          styling: isHighlighted ? {
            bgColor: resource.styling?.bgColor || 'bg-green-50',
            borderColor: resource.styling?.borderColor || 'border-green-200',
            iconBg: resource.styling?.iconBg || 'bg-green-100',
          } : undefined
        };
      })
    : defaultResources;
  ...
  return (
    <div className="bg-gray-50 pb-16 px-4 sm:px-6 lg:px-8 w-full overflow-x-hidden">
      <div className="max-w-7xl mx-auto w-full">
        
        {/* Header Section */}
        <div className="text-center w-full pt-24">
          <h1
            className="mx-auto max-w-4xl text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-semibold tracking-tight text-gray-900 px-4 pt-18"
